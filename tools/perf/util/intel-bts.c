...
static int intel_bts_synth_error(struct intel_bts *bts, struct perf_tool *tool,
				 int cpu, pid_t pid, pid_t tid, u64 ip)
{
	union perf_event event;
	...
	if (!branch->from) {
		...
		if (err) {
			...
			if (!btsq->bts->synth_opts.errors)
				return 0;
			err = intel_bts_synth_error(btsq->bts, tool, btsq->cpu,
						    btsq->pid, btsq->tid,
						    branch->from);
			return err;
		}
				    ...
				    struct perf_tool *tool)
{
	...
	for (; sz > bsz; branch += 1, sz -= bsz) {
		if (!branch->from && !branch->to)
			continue;
		intel_bts_get_branch_type(btsq, branch, tool);
...
static int intel_bts_process_queue(struct intel_bts_queue *btsq, u64 *timestamp,
				   struct perf_tool *tool)
{
	struct itrace_buffer *buffer = btsq->buffer, *old_buffer = buffer;
	struct itrace_queue *queue;
	struct thread *thread;
	int err;
